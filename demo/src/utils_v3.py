...
class ParkingDatabaseManager:
    ...
    def update_space_status(self, parking_spaces_id, estado):
        ...
        try:
            ...
            query_select = """
                SELECT id FROM state_date WHERE parking_spaces_id_sd = %s AND fecha = %s
            """
            cursor.execute(query_select, (parking_spaces_id, fecha_actual))
            result = cursor.fetchone()

            if result:
                # Actualiza el estado si ya existe un registro para la fecha
                query_update = """
                    UPDATE state_date
                    SET estado = %s
                    WHERE parking_spaces_id_sd = %s AND fecha = %s
                """
                cursor.execute(query_update, (estado, parking_spaces_id, fecha_actual))
            else:
                # Inserta un nuevo registro si no existe
                query_insert = """
                    INSERT INTO state_date (parking_spaces_id_sd, fecha, estado)
                    VALUES (%s, %s, %s)
                """
                cursor.execute(query_insert, (parking_spaces_id, fecha_actual, estado))
